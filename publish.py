...
# ---- R2 图床配置 ----
R2_BUCKET  = "yeyi-ai-image"          # 桶名
R2_BASE_URL = "https://img.ktzwei.top" # 绑定的自定义域名
R2_REMOTE  = "r2"                     # rclone 里配置的 remote 名
from urllib.parse import quote

def upload_to_r2(local_path, remote_path):
    """把本地图片传到 R2，返回公网 URL。remote_path 不含桶名，如 ict/01/xx.png。
    若目标已存在则跳过（不重复上传），节省流量。"""
    dest = f"{R2_REMOTE}:{R2_BUCKET}/{remote_path}"
    # 已存在则跳过
    exist = subprocess.run(['rclone', 'lsf', dest],
                           capture_output=True, timeout=30)
    if exist.returncode != 0 or not exist.stdout.strip():
        subprocess.run(['rclone', 'copyto', local_path, dest],
                       check=True, capture_output=True, timeout=120)
    return f"{R2_BASE_URL}/{quote(remote_path)}"
CSS = """
<style>
...
def md_to_html(md_text, prefix=None):
    """极简 markdown 转换（够用即可）。"""
    lines = md_text.split('\n')
    html = []
    i = 0
    ...
    while i < len(lines):
        ...
        if m:
            ...
            html.append(re.sub(r'!\[([^\]]*)\]\(([^)]+)\)',
                lambda mm: f'<figure style="margin:22px 0"><img src="{upload_to_r2(os.path.join(ASSETS,os.path.basename(mm.group(2))), f"{prefix}/{os.path.basename(mm.group(2))}")}" alt="{mm.group(1)}"/><figcaption style="color:var(--muted);font-size:.9em;text-align:center">{mm.group(1)}</figcaption></figure>',
                ln.strip()))
            i += 1; continue
        s = ln.strip()
        if not s:
...
def build_lesson(note_md, title, num, push=False):
    ...
    body = md_to_html(inline_images(md, ASSETS, f"ict/{num}"), f"ict/{num}")
